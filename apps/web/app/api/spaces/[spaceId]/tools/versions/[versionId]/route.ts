...
/**
 * The two decisions a space admin makes about a version of their own Tool.
 *
 * `review` is the update queue's verdict — approving is what makes the code
 * installable here and offers it to this space's existing installs. `list` and
 * `unlist` are the marketplace, and they are deliberately a SEPARATE act on a
 * version that is already approved: publishing ships a Tool to the people who
 * wrote it, and asking the world to run it is a different sentence with a
 * different reviewer at the end of it.
 *
