...
const Settings = z.object({
  ...
  inputs: z.record(z.string(), z.string().max(500)).nullish(),
})

/**
 * Who the agent runs for — the brief's `for:` block. POST puts YOUR name down,
 * with your own time, zone, model and input values when you give them — each fire then runs
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ spaceId: string; name: string }> }) {
  ...
  return r.ok ? NextResponse.json({ ok: true, warning: r.warning }) : bad(r.error, r.status)
}
