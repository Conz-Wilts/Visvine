// Access requests for a space's shared context (successor of
...
import { resolveReferenceToken } from '@/lib/notes/contextService'

export async function GET(req: NextRequest) {
  const context = await requireContext(req)
  if (context instanceof Response) return context
  if (context.isPersonalSpace) return NextResponse.json({ requests: [], pending: 0 })
...
export async function POST(req: NextRequest) {
  ...
  if (context instanceof Response) return context
  if (context.isPersonalSpace) return fail('Personal spaces are private — there is nothing to request')
  // '' is the ROOT GATE (a request for context access) and a valid resourcePath.
  ...
  const p = await principalOf(context)
  if (resourcePath === null && referenceToken && targetPath) {
    resourcePath = await resolveReferenceToken(p, context, targetPath, referenceToken)
