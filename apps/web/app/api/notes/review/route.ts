...
//   { communityId, scope, mode: 'light'|'full', apply? } → { report, applied }
...
// brain restructures community content, so it's community-admin only; a personal
...
export async function POST(req: NextRequest) {
  ...
  if (brain.scope === 'shared' && !brain.isAdmin) {
    return fail('Only an admin can run a review over the community brain', 403)
  }
