...
// The graph pulls in d3-force + the canvas renderer. Defer the whole thing so
...
/**
 * The Context tool: the community graph, full-bleed and immersive, with the
...
export default function ContextPage() {
  ...
  // The search's best-matching node, lifted out of the graph so the docked
  ...
  // page must inset itself or the panel covers the search bar and graph edge.
  ...
  // The graph owns its surface: fixed-height container, no page scroll.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Graph canvas — persisted layout restores on remount, so navigating away
