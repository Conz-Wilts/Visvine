...
function mkNodes(n: number, r?: number): ContextNodeInput[] {
  return Array.from({ length: n }, (_, i) => ({ id: `n${i}`, r }));
}

function seededRandomContext(
  n: number,
  m: number,
  seed = 42,
): { nodes: ContextNodeInput[]; edges: ContextEdgeInput[] } {
  ...
  const edges: ContextEdgeInput[] = [];
  // Spanning tree first so the context is connected, then extra random edges.
  for (let i = 1; i < n; i++) {
...
test('empty context returns an empty, well-formed result', () => {
  const res = layoutContext([], []);
  assert.deepEqual(res.nodes, []);
  assert.equal(res.stats.overlapPairs, 0);
});

test('single node is centered in the viewport', () => {
  const res = layoutContext([{ id: 'only' }], [], { width: 1000, height: 600 });
...
test('two connected nodes sit roughly an ideal edge length apart', () => {
  const res = layoutContext(mkNodes(2), [{ source: 'n0', target: 'n1' }], {
    idealEdgeLength: 50,
    maxUpscale: 1,
  });
  const [a, b] = res.nodes;
...
test('all nodes and edges are present in the output', () => {
  const { nodes, edges } = seededRandomContext(120, 200);
  const res = layoutContext(nodes, edges);
...
test('zero node-node overlap on a mid-size random context', () => {
  const { nodes, edges } = seededRandomContext(400, 700);
  const res = layoutContext(nodes, edges);
...
test('isolated nodes settle on the periphery of the cloud, overlap-free', () => {
  const { nodes, edges } = seededRandomContext(80, 140);
...
test('determinism: identical input produces identical output', () => {
  const { nodes, edges } = seededRandomContext(200, 350, 7);
  const r1 = layoutContext(nodes, edges);
  const r2 = layoutContext(nodes, edges);
...
test('seed option changes the layout, same seed reproduces it', () => {
  const { nodes, edges } = seededRandomContext(100, 160, 3);
  const a = layoutContext(nodes, edges, { seed: 'alpha' });
  const b = layoutContext(nodes, edges, { seed: 'alpha' });
  const c = layoutContext(nodes, edges, { seed: 'beta' });
  assert.deepEqual(
...
test('crossing reduction: ladder context ends up with few crossings', () => {
...
test('performance: 2000 nodes / 4000 edges completes inside 3 seconds', () => {
  const { nodes, edges } = seededRandomContext(2000, 4000, 11);
  const t0 = performance.now();
  const res = layoutContext(nodes, edges, { timeBudgetMs: 2500 });
...
test('performance: 5000 nodes / 10000 edges respects the budget and stays overlap-free', () => {
  const { nodes, edges } = seededRandomContext(5000, 10000, 13);
  const t0 = performance.now();
  const res = layoutContext(nodes, edges, { timeBudgetMs: 2500 });
  const wall = performance.now() - t0;
...
test('async driver emits progressive snapshots and matches the contract', async () => {
  const { nodes, edges } = seededRandomContext(600, 1000, 5);
  const phases: string[] = [];
  let snapshotNodes = 0;
  const res = await layoutContextAsync(nodes, edges, {
    onProgress: (snap) => {
...
test('timeBudgetMs=0 still returns a complete overlap-free layout', () => {
  const { nodes, edges } = seededRandomContext(800, 1400, 21);
  const res = layoutContext(nodes, edges, { timeBudgetMs: 0 });
...
test('honors viewport and padding for everything including the shelf', () => {
  const { nodes, edges } = seededRandomContext(150, 250, 9);
  for (let i = 0; i < 40; i++) nodes.push({ id: `iso${i}` });
  const W = 900;
  const H = 500;
  const res = layoutContext(nodes, edges, { width: W, height: H, padding: 20 });
