...
// `getFeatureConfig` lives in lib/auth.ts, next to the other space reads.
//
...
// tool is switched back on — this filters what a space SHOWS, not what it
...
/** Keep only the nodes whose type belongs to a tool this space still has on. */
...
/**
 * The nodes a space still shows, plus only the links whose BOTH ends
