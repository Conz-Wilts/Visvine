...
/** Bumped only for a breaking wire change; the frame reports the version it speaks. */
 ...
 * because only the first is theirs to fix.
 */
...
export interface BridgeMethods {
  ...
  'state.set': { params: { key: string; value: unknown }; result: null }
...
export type FrameMessage =
  ...
  | { type: 'visvine:call'; id: string; method: BridgeMethod; params: unknown }
...
function isDegraded(value: unknown): value is ToolDegraded | null {
  ...
  const { connectors, types, agents } = value.missing
  return isStringArray(connectors) && isStringArray(types) && isStringArray(agents)
}
...
export function isFrameMessage(value: unknown): value is FrameMessage {
  ...
  switch (value.type) {
    ...
    case 'visvine:call':
      return typeof value.id === 'string' && isBridgeMethod(value.method) && 'params' in value
    case 'visvine:resize':
