...
export async function ensureMemberNode(
   ...
   * already there: a member's chip is space-local and editable, and a
   * re-join must not overwrite it.
   */
  alias?: string | null,
): Promise<string | null> {
  // Already connected somewhere in this space — done, whatever the node is
  // named locally (renames are space-local and must survive re-joins).
