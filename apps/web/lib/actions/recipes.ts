...
const RECIPES: Recipe[] = [
...

  {
    ...
    mustKnow: (ctx) => [
      ...
      'create_agent CREATES only. An existing name is refused rather than overwritten — a live agent runs the brief a person approved. Its prose is edited on the note; how it runs (model, connectors, tools) with configure_agent — never as frontmatter.',
...

  {
    ...
    intake: 'tool',
    steps: (ctx) => [
      { n: 1, tool: 'plan_tool', why: 'Research first: what the space holds (records, its own types and their fields, folders, Tools), the traps in the request, where each thing should live, and the plan template.', args: { space_id: spaceId(ctx), request: '<what the person asked, verbatim>' } },
      { n: 2, tool: 'get_tool_sdk', why: 'The authoring contract — the kit, the bridge, the manifest. Read once.', args: {} },
      { n: 3, tool: '(ask the person)', why: 'Fill in plan_template — views, band actions, data and where it lives, controls, charts, images, icon, reach — and show it in ONE message. Build on their yes, or on "you decide".', args: {} },
      { n: 4, tool: 'add_type', why: 'Only when the plan gives a thing a note type of its own: the type with its fields (a select field carries its options).', args: { space_id: spaceId(ctx), name: '<Type>', fields: [{ key: '<key>', label: '<Label>', kind: 'text' }] }, optional: true },
      { n: 5, tool: 'create_tool', why: 'Scaffolds the Tool with the agreed plan as its ## Design section.', args: { space_id: spaceId(ctx), name: '<name>', title: '<Display name>', description: '<one sentence>', plan: '<the filled-in template>' } },
      { n: 6, tool: 'configure_tool', why: 'Everything the plan decided that is structure: surfaces (nav sections, band actions), collections with their enums, bindings, permissions.', args: { space_id: spaceId(ctx), name: '<name>', facts: { surfaces: {}, collections: {}, permissions: {} } } },
      { n: 7, tool: 'set_tool_icon', why: 'A built-in name, or your own 24×24 stroke SVG (or the one the person attached, via upload_file).', args: { space_id: spaceId(ctx), name: '<name>', icon: 'chart' } },
      { n: 8, tool: 'write_tool', why: 'ui.tsx: write, read the diagnostics, write again.', args: { space_id: spaceId(ctx), name: '<name>', file: 'ui.tsx', content: '<source>' } },
      { n: 9, tool: 'check_tool', why: 'Compile, lint, design findings and a headless render — fix every design.* finding.', args: { space_id: spaceId(ctx), name: '<name>', render: true } },
      { n: 10, tool: 'preview_tool', why: 'A screenshot per section (`section`) and per band action (`action`). Read each against its `review` list and fix what fails before handing over the link.', args: { space_id: spaceId(ctx), name: '<name>', screenshot: true } },
