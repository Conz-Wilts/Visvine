 ...
 *
 * One authorization-server document and one protected-resource document. The
 * resource document is served from two paths (the origin root, RFC 9728
 * path-suffixed, and the MCP endpoint's own sub-path), because MCP clients
 * probe both; they must be byte-identical, which is easiest to guarantee by
 * building them here.
 */
