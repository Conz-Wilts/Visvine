...
const GUIDANCE: Record<string, string> = {
  ...
  connector: 'A gateway to an external API or database, note-first and admin-only: an admin authors connectors/<name>.md (frontmatter declares alias/hosts/limits). Never creatable via add_context; execute one with run_connector. The models agents run on are not connectors: they are `type: model` notes under models/ (list_models).',
  agent: 'A scheduled agent, note-first and a folder: agents/<name>/index.md IS the agent — frontmatter says what it is (model, connectors, tools) and when it runs (active, schedule/every/on), the body is its instructions — and the rest of agents/<name>/ is what its runs write. Not creatable via add_context — use create_agent, then activate_agent to turn it on (anyone who can edit the brief; creating one does not start it). List with list_agents, trigger with run_agent.',
  ...
  model: 'An LLM the space\'s agents run on, note-first and admin-only: models/<name>.md (frontmatter declares provider and model id; its key is a space secret). Never creatable via add_context and never runnable — list with list_models, and name one in an agent\'s `model:`.',
}
