...
const GUIDANCE: Record<string, string> = {
  ...
  agent: 'A scheduled agent, note-first and a folder: agents/<name>/index.md is the brief (frontmatter: model, connectors, tools; body = the instructions), agents/<name>/activation.md beside it is the admin activation, and the rest of agents/<name>/ is what its runs write. Not creatable via add_context — use create_agent, then activate_agent to turn it on (admins only; creating one does not start it). List with list_agents, trigger with run_agent.',
