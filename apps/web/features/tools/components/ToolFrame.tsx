...
/** How long the frame has to complete its handshake before we call it broken. */
...
export default function ToolFrame({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setError('This Tool did not finish loading. It may be failing to start, or its bundle may be missing.');
    }, READY_TIMEOUT_MS);
      ...
      <div ref={slotRef} className="relative w-full" style={{ height: error ? undefined : frameHeight }}>
        {error ? (
          ...
          <>
            ...
            {status !== 'ready' && <ToolFrameSkeleton />}
          </>
        )}
      </div>
    </div>
