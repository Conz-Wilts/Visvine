...
/**
 * A Resource's file — the Preview tab of a resource node whose `metadata.fileId`
 * names one. The viewer is the hero; a header carries the file's identity, its
 * index state and Download, and a right-hand activity panel holds Comments,
 * proposed Changes (spreadsheets) and Details. The node page is the file's
 * address, so sharing and deleting are the page's, not this view's.
 */
...
export default function ResourceFile({ resourceId, className = 'h-[calc(100dvh-56px)]' }: {
  resourceId: string;
  /** Sizes the view; it fills that box and scrolls inside it. */
  className?: string;
}) {
  ...
  const canReview = isAdmin || !!detail?.viewer.canManage;
  ...
  return (
    <div className={`flex ${className} w-full flex-col`}>
      {/* ── identity + actions ── */}
