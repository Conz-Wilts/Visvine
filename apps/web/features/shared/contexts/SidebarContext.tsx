...
interface SidebarContextValue {
   ...
   *  It is also held open while one of its menus is showing (`menuOpen`).
  ...
  /** A menu anchored to a rail row is open — the space menu at its head, the
   *  account menu at its foot (components/ui/Popover). They portal to <body>,
   *  so the pointer crossing into one has LEFT the rail; this holds the rail
   *  open under them, so the row the menu hangs off stays where it was. */
  menuOpen: boolean;
...
export function SidebarProvider({ children }: { children: ReactNode }) {
  ...
  // the shell's own chrome — the space menu and the tool list — so it costs
  ...
  return (
    <SidebarContext.Provider value={{ expanded: hovered || menuOpen, setHovered, menuOpen, setMenuOpen, reduced }}>
      {children}
