...
/**
 * The page for a Community node that is a RECORD — a community, company or
 * organisation kept in someone else's graph for CRM purposes, with no
 * Community row of its own behind it.
 *
 * Every Community node reads as a community page; only the data source differs.
 * A node standing for a community that actually runs here (its own root node, or
 * a record carrying `metadata.communityRef`) is sent to /communities/<id> by
 * CommunityRoute and renders from the overview API. Everything else lands here
 * and renders from the node — same visual language as that page (cover, hero
 * avatar, Community badge, stat strip, About, At-a-glance rail) so the two are
 * recognisably the same kind of page rather than a community page and an
...
export default function CommunityPageContent({ nodeId, onConnectionsClick }: CommunityPageContentProps) {
  ...
  const typeLabel = nodeTypeLabel(node.type, node.alias, currentCommunity?.communityAliases, currentCommunity?.nodeTypes);

  return (
    <div className="profile-content-fade flex flex-col gap-5">
      {/* ══ COVER + HERO — the community page's shape, inside the pane's gutter
        ...
        <div className="relative px-5 sm:px-8 pb-5">
          <div className="absolute -top-14 left-5 sm:left-8">
            {/* Contained, not cropped: a logo with whitespace must not be zoomed
                to fill, so the object-fit differs from the member-community page
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_300px] xl:grid-cols-[minmax(0,1fr)_320px] gap-5 xl:gap-6">
        ...
        <div className="flex flex-col gap-4 lg:sticky lg:top-16 self-start">
          ...
          {/* The network teaser the member community page carries, minus the
