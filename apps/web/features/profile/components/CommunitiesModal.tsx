...
interface Props {
  ...
  /** Viewing your own profile — shows the per-community visibility toggles. */
  isOwner: boolean;
  personName: string;
  theme: ThemePalette;
  /** Owner only: persist a member community's show-on-profile flag. */
  onToggle?: (communityId: string, showOnProfile: boolean) => Promise<void>;
}
...
export default function CommunitiesModal({ open, onClose, communities, isOwner, personName, theme, onToggle }: Props) {
  ...
  const managed = communities.filter((c) => c.role === 'admin');
  const memberOf = communities.filter((c) => c.role !== 'admin');

  const toggle = async (c: ProfileCommunity) => {
          ...
          {managed.length > 0 && (
            <section>
              ...
              <div className="flex flex-col gap-1.5">
                {managed.map((c) => (
                  <CommunityRow key={c.id} community={c} theme={theme}
                    trailing={isOwner
            ...
            <section>
              ...
              <div className="flex flex-col gap-1.5">
                {(isOwner ? memberOf : memberOf.filter((c) => c.visible)).map((c) => (
                  <CommunityRow key={c.id} community={c} theme={theme} dimmed={isOwner && !c.showOnProfile}
                    trailing={isOwner ? (
...
function CommunityRow({ community: c, theme, trailing, dimmed }: {
  community: ProfileCommunity; theme: ThemePalette; trailing: React.ReactNode; dimmed?: boolean;
}) {
  return (
