...
| `context_records` | A note that declares one of the space's invented types (`scope: note`) — a record the Directory's table lists and `list_records` queries: its path, type, title, tags and when it was saved. Derived from the note on every write, dropped with it. |
| `context_record_fields` | One field value of such a record: what its note's frontmatter says under a key the type declares, typed by the field's kind into a text, number, date or boolean column — or `invalid` with the raw text when it does not read as that kind. Re-read when the type's fields change. |
...
| `app_tool_configs` | A Tool's manifest facts, one row per `(space, name)`: surfaces, reach (`permissions`, or a v1 `perimeter`), bindings, settings, the kit range, dependencies, collections. The index note keeps the prose; every reader composes the two. |
| `app_tool_config_changes` | Every key of a Tool's facts that moved: before, after, who. |
| `app_tool_versions` | Every published version: an immutable snapshot of config, perimeter, sources (the UI's `src/` modules in `modules`) and bundles, numbered from 1 per Tool key. Two verdicts — `status` (the source space's) and `marketplace_status` (Visvine's, null until someone asks) — and a withdrawal (`revoked_at`) that stops the version wherever it runs. |
| `app_tool_installs` | A space running one version: its slug and rail key, what it is missing (`requirements`, unbound slots included), which type pages or tabs it owns, what each binding slot is bound to here (`bindings`) and its `settings`, an offered upgrade, and the house it came down from when shared into a room. |
| `app_tool_state` | A Tool's small key/value store per install (UI preferences, not space data): `user_id` `''` for the value every viewer shares, a viewer's id for their own (cleared with their account). Cascades with the install. |
| `app_tool_deploy_keys` | A Tool's deploy keys — CI's MCP bearer for one Tool in one space, acting as whoever minted it: the label, the shown prefix, the SHA-256 (never the key), last use, and a revocation. Go with the Tool, the space or the minter's account. |
| `app_tool_records` | The rows of a Tool's collections: one per row, keyed by what the Tool runs as (`target_key`: the install, `preview:<space>/<name>`, or a review run), the Tool's key it is matched back by, the collection, the JSON the schema held it to, and who wrote it (never shown to a Tool; goes with their account). An uninstall detaches rows (`detached_at`) for a reinstall to take back; the tick purges them after 30 days. |
| `app_tool_listings` | A Tool's global listing, named by an id that survives a transfer: the key it names now, its publisher space, the co-signing author and license, when it was first listed and its stage (`staged_until`, `staged_cap`), a transfer on offer — and Visvine's hold over it: `active`, `suspended` or `revoked`, set by a reviewer or suspended by monitoring. A hold stops every install outside the publisher's family. |
| `app_tool_publishers` | A publishing space as Visvine knows it: whether a reviewer verified it, by whom and when. A verified publisher's later listings start unstaged, and its unchanged, clean re-listings are listed without a person. Kept when the space goes. |
| `app_tool_telemetry` | What each install of a Tool did, as counts — calls and refusals by method and code, bytes, paths, CSP reports, navigations, frame errors, viewers — one row per install, day and server instance (never content), rolled into one row per day when the day is over and kept 90 days. |
| `app_tool_advisories` | Known advisories (OSV) against the curated dependencies at the versions Visvine vendors; the security stage flags a Tool that declares one, and a new one triggers a rescan. |
| `app_tool_check_runs` | One stage of the automated checks per run — compatibility or security, and for a version offered for listing Visvine's `ai` and `dynamic` — on a working copy (`version_id` null, the newest few kept) or on the version a publish wrote. Status, findings, the risk score and the analyzer that ran; read as written by the author, the admins and Visvine's reviewers alike. |
| `app_tool_review_runs` | Visvine's global stages over one co-signed listing request, as a queued job the minute tick drains: its status, the browser that ran it, the honeypot space made for it (deleted when it ends) and the canaries planted there. |
| `app_tool_review_events` | What a Tool did during its dynamic run — each bridge call, each door out of the space it asked for (recorded, never opened), each CSP report, navigation and egress attempt. The evidence the run's scan reads. |
| `app_tool_consents` | A member's Continue on a listed Tool's first-use notice, per install and person, with the acting reach it covered; an upgrade that widens it asks again. Goes with the person's account. |
| `app_tool_incidents` | What a Tool did that a person should look at — a frame that navigated itself, a CSP violation, a report, an anomaly, a rescan's new finding, a dynamic run's catch — naming its listing and its source (viewer, dynamic, monitor, rescan), and how a reviewer resolved it. Facts about the Tool only; the viewer is dropped when they delete their account. |
