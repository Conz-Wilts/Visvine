...
| `app_tool_listings` | A Tool's global listing, named by an id that survives a transfer: the key it names now, its publisher space, the co-signing author and license, Visvine's `verified` word, when it was first listed and its stage (`staged_until`, `staged_cap`), a transfer on offer — and Visvine's hold over it: `active`, `suspended` or `revoked`. A hold stops every install outside the publisher's family. |
