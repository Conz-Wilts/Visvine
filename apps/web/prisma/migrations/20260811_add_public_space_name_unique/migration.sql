...
--
-- The rule is enforced in the app (lib/communities/publicName.ts) on every write
...
-- lib/communities/publicName.ts, or the app check and this index disagree.
...
CREATE UNIQUE INDEX IF NOT EXISTS communities_public_name_unique
  ON communities (lower(regexp_replace(btrim(name), '\s+', ' ', 'g')))
