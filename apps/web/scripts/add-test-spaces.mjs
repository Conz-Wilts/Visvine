// Adds 8 extra "test" spaces and joins the two dev anchor users
// (user_dev_admin, user_dev_member) to each, so the space-selector
// dropdown has enough rows to exercise the scroll (cap is 4 visible).
//
// Idempotent: re-running upserts the spaces and skips memberships
...
const connectionString =
  ...
  (process.env.DB_HOST
    ...
    : null);
if (!connectionString) throw new Error('add-test-spaces: no DATABASE_URL or DB_HOST resolved from apps/web/.env');
...
const SPACES = [
  ...
  { id: 'community:test-500',       name: '500 Global',           description: 'Early-stage venture fund and accelerator with a global founder space.',            location: 'San Francisco, USA',  tags: ['VC', 'Accelerator'],              country: 'US' },
...
try {
  ...
  console.log(`--- Upserting ${SPACES.length} test spaces + joining ${ANCHORS.length} anchor users ---`);
  for (const c of SPACES) {
    await client.query(
      `
      INSERT INTO spaces (id, name, description, location, tags, country, visibility, created_at)
    ...
    for (const a of ANCHORS) {
      await client.query(
        `
        INSERT INTO space_members (user_id, space_id, status, joined_at)
      ...
      if (a.owner) {
        // The schema default for aliases already carries the built-in
  ...
  const total = await client.query(
    `SELECT COUNT(*)::int AS n FROM space_members WHERE user_id = 'user_dev_admin'`,
  );
  console.log(`\nDone. Dev Admin is now in ${total.rows[0].n} spaces.`);
} catch (err) {
