 ...
 */
function InviteLinkCard({ communityId }: { communityId: string }) {
  ...
  return (
    <div className="rounded-xl border border-border-default bg-surface-2 p-4 space-y-2">
      ...
      <p className="text-xs text-text-muted">
        Share this link to invite people. New joiners appear below as pending requests for you to approve.
      </p>
      <div className="flex items-center gap-2">
        <input
          readOnly
          value={loading ? 'Loading…' : url}
          className="flex-1 px-3 py-2 text-xs border border-border-default rounded-lg bg-surface-1 text-text-secondary truncate"
        />
        <button
          onClick={copy}
          disabled={loading || !url}
          className="px-3 py-2 text-xs font-medium text-white rounded-lg transition-colors disabled:opacity-50"
...
export default function MembersPanel({ communityId }: { communityId: string }) {
  ...
  const denyMember = async (userId: string, name: string) => {
    if (!confirm(`Deny ${name}'s request to join?`)) return;
    setActionLoading(userId);
    try {
      const res = await fetch(`/api/communities/${communityId}/members/${userId}`, { method: 'DELETE' });
      const data = await res.json();
  ...
  return (
    <div className="space-y-4">
      <InviteLinkCard communityId={communityId} />
              ...
              <div key={member.id} className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-text-muted truncate">{member.user.email}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="px-2.5 py-1 rounded-full text-xs font-medium bg-brand-green text-white hover:opacity-90 transition disabled:opacity-50"
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => denyMember(member.userId, member.user.name)}
                    disabled={actionLoading === member.userId}
                    className="px-2.5 py-1 rounded-full text-xs font-medium border border-border-default text-text-secondary hover:bg-surface-2 transition disabled:opacity-50"
                  >
                    Deny
                  </button>
                </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-text-muted">{activeMembers.length} member{activeMembers.length !== 1 ? 's' : ''}</p>
        <button
