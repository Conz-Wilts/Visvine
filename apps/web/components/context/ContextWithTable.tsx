...
const ContextWithTable: React.FC<ContextWithTableProps> = ({
  ...
  // Create a hash of the context structure to detect when it actually changes
  const contextDataHash = useMemo(() => {
    const nodeIds = contextData.nodes.map(n => n.id).sort().join(',');
    const linkIds = contextData.links.map(l => `${l.source}-${l.target}`).sort().join(',');
    return `${nodeIds}::${linkIds}`;
  }, [contextData.nodes, contextData.links]);
  ...
  const layoutHash = `${LAYOUT_ALGO_VERSION}:${contextDataHash}`;
  ...
  const incrementalLayout = useMemo<Map<string, { x: number; y: number }> | null>(() => {
    if (serverSeed !== null || contextData.nodes.length === 0) return null;
  ...
  }, [serverSeed, contextData.nodes, simLinks, initialLayout]);
  ...
  const engineLayout = useMemo<Map<string, { x: number; y: number }> | null>(() => {
    if (serverSeed !== null || incrementalLayout !== null || contextData.nodes.length === 0) return null;
  ...
  }, [serverSeed, incrementalLayout, contextData.nodes, simLinks, nodeTypes, contextDataHash]);
  ...
  }, [contextData.nodes, contextDataHash, serverSeed, incrementalLayout, engineLayout]);
