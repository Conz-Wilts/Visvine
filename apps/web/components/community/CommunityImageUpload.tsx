...
export default function CommunityImageUpload({
  ...
  const hasExisting = !!community.imageUrl;
  ...
  async function handleSave() {
    ...
    try {
      // Delete old image variants if one exists
      if (hasExisting) {
        await deleteImage('community', community.id);
      }

      const url = await uploadImage('community', community.id, pendingFile);

      // Persist imageUrl to the community record
      const res = await fetch('/api/data/communities', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ community: { ...community, imageUrl: url } }),
      });
    ...
    <div className="flex flex-col gap-3">
      ...
      {preview ? (
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <CommunityAvatar name={community.name} imageUrl={community.imageUrl} size={size} />
            {community.imageUrl && (
