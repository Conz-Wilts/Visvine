...
| `FileTypeIcon` | one per file kind: `image video audio pdf doc sheet slides text code archive link other` | `sm` 28, `md` 36, `lg` 56, `xl` 96 | the kind's glyph in `color.file.<kind>.fg` on `.wash`; the one mark for a file anywhere |
...
- **`FileTypeIcon`** — the kind's glyph (the `file-*` icons, generated for both platforms) in `color.file.<kind>.fg` on `.wash`. Sizes 28 / 36 / 56 / 96 pt with the web's corner radii (`md`, `lg`, `xl`, `2xl`).
