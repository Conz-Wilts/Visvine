...
| `Logo` | `mark` (brand green glyph), `tile` (white glyph on the brand-green rounded square) | `size` in px | the logo as SVG, painted in `brand`, never the accent; `title` names it |
...
| `IconButton` | — | `sm` 28, `md` 32 | an icon that is the whole control; `label` is its accessible name and title; `active` when pressed |
| `Menu` | — | — | a popover of actions under a trigger (the toolbar's …); arrow keys, Enter, Escape; `danger` items in the danger ink; floats, so it casts `float` |
| `FileTypeIcon` | one per file kind: `image video audio pdf doc sheet slides text code archive link other` | `xs` 20, `sm` 28, `md` 36, `lg` 56, `xl` 96 | the kind's glyph in `color.file.<kind>.fg` on `.wash`; the one mark for a file anywhere |
| `ResourceRow` | `comfortable`, `compact` | — | thumbnail or kind tile, name, one `·` meta line; actions on hover; `selected` in `accent.soft`; a `select` slot for bulk choice |
| `ResourceCard` | `file` (4:3 picture over name and meta), `image` (square, name on hover) | — | falls back to the kind tile when there is no picture; `selected` is an accent ring |
| `ResourceGrid` | `file`, `image` (denser) | — | auto-fill columns at least a card wide |
| `UnfurlCard` | `large` (wide image above), `summary` (square beside) | — | a link as Slack lays it: site line, title, description, picture; `pending` is a skeleton; opens the viewer through `onOpen`, else the page |
| `ResourceViewer` | `panel` (side sheet, 28–30rem), `full` (the window, rounded, over a scrim) | — | header (tile or avatar, title, meta), one labelled primary, icon actions, a `Menu`; prev/next and position; zoom controls; keys ← → Esc f + − 0; focus in on open, back on close |
| `ViewerFallback` | — | — | a file the viewer cannot draw: its `xl` tile, name, meta and the ways to open it — never an apology |
| `ZoomPane` | — | — | zoom toward the cursor (⌘/ctrl + wheel, pinch, double-click), drag to pan only past fitting; the numbers are `zoomMath` (25% steps; images to 200%, documents to 300%) |
...
#### The resources viewer, natively

The phones draw resources to the same spec (`docs/resources/mobile.md` holds the screens and the API):

- **`FileTypeIcon`** — the kind's glyph (the `file-*` icons, generated for both platforms) in `color.file.<kind>.fg` on `.wash`. Sizes 20 / 28 / 36 / 56 / 96 pt with the web's corner radii (`DEFAULT`, `md`, `lg`, `xl`, `2xl`).
- **`ResourceRow` / `ResourceCard`** — the same anatomy and the same `·` meta line; a long-press is the hover actions.
- **`UnfurlCard`** — the same layout; tapping it opens the resource.
- **`ResourceViewer`** — the platform's own full-screen viewer, not a re-drawn one: iOS `QLPreviewController` over the downloaded original (links in `SFSafariViewController`), Android an `ACTION_VIEW` intent on a `FileProvider` URI (links in a Custom Tab). The header's actions become the share sheet.
