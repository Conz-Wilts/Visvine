 ...
 *
 *   fixtures/space.json    who is looking, the install's bindings and settings,
 *                          and what connectors, actions and the AI answer
...
export function loadFixtures(root: string): FixtureSpace {
  ...
  const viewer = recordOf(config.viewer)
  ...
  return {
    viewer: {
      id: typeof viewer.id === 'string' ? viewer.id : 'viewer',
      name: typeof viewer.name === 'string' ? viewer.name : 'You',
