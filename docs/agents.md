...
| `decide {items[], questions[]}` | whenever there is a judge | the agent's own yes/no, choice and scale questions put to the judge for up to 48 items at once — triage and routing without a turn per item. Numbers back, nothing else: no write, grant or gate reads them. Metered per space (`takeSpaceJudgeAllowance`) |
