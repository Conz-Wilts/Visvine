...
- *The rewrite gate as designed did not work.* Asked whether a query could be
  ...
  rewrite only when the judge found nothing strong — and then the rewrite was
  removed outright: every caller of search is a model that rephrases and
  searches again on its own.
