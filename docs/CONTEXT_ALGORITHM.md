# Context Layout & Rendering Algorithm

How Visvine turns a community's `{ nodes, links }` into the interactive node/link
context you see on screen. The product surface is a force-directed layout of cards
...
The key design choice: **the context does not continuously simulate.** Positions
...
| `components/context/utils/constants.ts` | All tuning constants (`CARD_DIMENSIONS`, `LOD_THRESHOLDS`, `OBSIDIAN_PHYSICS`) |
| `components/context/utils/forceRectCollide.ts` | Custom rectangular collision force for cards |
| `components/context/utils/incrementalLayout.ts` | Reuse a saved layout and place only new nodes |
| `components/context/hooks/useLayoutPersistence.ts` | Debounced persistence of positions + camera |
| `components/context/renderers/*` | Per-shape node renderers + link renderer (LOD-aware) |
| `app/api/communities/[communityId]/context/layout/route.ts` | GET/PUT saved layout (positions + camera) |
...
API/DB: { nodes, links }
   │
   ▼
DirectoryContextView ── fetch + text-search dimming/focus
   │
   ▼
ContextWithTable ── compute structure hash, then choose a layout SOURCE:
   ├─ server seed?      (hash + algo version match)  → restore positions + camera
   ├─ incremental?      (≥70% of nodes already placed) → reuse + place new nodes
   ├─ engine layout?    (fresh)                         → layoutContext(...)
...
simNodes (x/y + spawn metadata)
   │
   ▼
ContextCanvas ── canvas render loop + interaction
...
Union-find splits the node/link graph into connected components. Isolated (degree-0) nodes
...
Engine options passed from `ContextWithTable.tsx`:
...
layoutContext(nodes, links, {
  ...
  timeBudgetMs: 1500,
  seed: `${contextDataHash}:${rerollNonce}`,  // deterministic; reroll forces a fresh seed
});
```

---

## 2. The runtime simulation & renderer (`ContextCanvas.tsx`)

### d3-force setup

When the context cold-starts (no usable saved/precomputed layout, or an explicit
...
the simulation is frozen. This is the central behavioural contract of the context.
...
New nodes fade in (700ms) and "rise" 26 world-units into place, staggered by 8ms
per node. The total stagger is capped at `fadeInMaxTotalStaggerMs = 1500` (spawn
indices are compressed at stamp time) so a 1000-node canvas doesn't redraw the
...
## 4. Choosing a layout source (`ContextWithTable.tsx`)
...
4. **Full engine** — `layoutContext(...)` from scratch.
...
## 7. Search & filtering (`DirectoryContextView.tsx`)

- Data comes from `useCommunityContextData()` → `{ nodes, links }`.
- **Text search** dims unmatched nodes (opacity ~0.15) rather than removing them,
  and focuses the best match. The full context stays visible, so the saved layout
  is unaffected.
...
5. **Cheap structure changes.** Incremental reuse places only new nodes instead
   of recomputing the whole context.
