...
- Directory search is fuzzy/keyword only (client-side relevance matching + server-side ILIKE pickers). The former *directory* semantic search was removed; context/notes search still uses pgvector embeddings when `OPENROUTER_API_KEY` is set.
